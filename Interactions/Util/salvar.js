...
module.exports = class SaveInteraction extends Interaction {
  constructor() {
    super("salvar", {
      type: 1,
      description: 'Salvar os arquivos das aulas',
  ...
  async execute({ interaction, client, db }) {
    ...
    let canalMembro = await interaction.guild.channels.create(`${interaction.user.username} - ${this.name}`,{
      ...
      reason: "Salvar um arquivo da escola",
    ...
    let messageButton = await canalMembro.send({
      embeds: [
        new MessageEmbed()
          .setDescription(`<@${authorId}> selecione o nome da matéria e quando quiser mudar é só mudar no menu. Envie o(s) arquivos e quando terminar aperte em 'Fim'. Os arquivos não podem ultrapassar \`16Mb\``)
          .addField("Reações", "✅: Arquivo salvo (pode ser também para o tópico)")
          .setColor('#2F3136')
          .setTimestamp()
      ],
    ...
    const collectorMes = canalMembro.createMessageCollector({ 
      filter: filterMessage, idle: 60000*2 
    })

    let topic;
    collectorMes.on('collect', async message => {
      if (message.content != "") await message.react("✅"), topic = message.content;
      try {
      ...
      try {
        if (i.customId === 'fim') {
          ...
          db(async function(clientMongo) {
            let db = clientMongo.db("arquivos")
            let collectionUser = db.collection(materia)
            if (!collectionUser) collectionUser = db.createCollection(materia, validator)
